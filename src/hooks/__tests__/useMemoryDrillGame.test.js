...
describe('Memory Drill Game Logic', () => {
  ...
  describe('Input Validation Patterns', () => {
    ...
    test('should validate PREFIX_TO_HEXTET input correctly', () => {
      ...
      validInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.PREFIX_TO_HEXTET, input)).toBe(true);
      });

      invalidInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.PREFIX_TO_HEXTET, input)).toBe(false);
      });
    });
    ...
    test('should validate HEX_TO_BINARY input correctly', () => {
      ...
      validInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.HEX_TO_BINARY, input)).toBe(true);
      });

      invalidInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.HEX_TO_BINARY, input)).toBe(false);
      });
    });
    ...
    test('should validate BINARY_TO_DECIMAL input correctly', () => {
      ...
      validInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.BINARY_TO_DECIMAL, input)).toBe(true);
      });

      invalidInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.BINARY_TO_DECIMAL, input)).toBe(false);
      });
    });
    ...
    test('should validate BINARY_TO_HEX input correctly', () => {
      ...
      validInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.BINARY_TO_HEX, input)).toBe(true);
      });

      invalidInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.BINARY_TO_HEX, input)).toBe(false);
      });
    });
    ...
    test('should validate IPV4_BINARY_TO_DECIMAL input correctly', () => {
      ...
      validInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.IPV4_BINARY_TO_DECIMAL, input)).toBe(true);
      });

      invalidInputs.forEach(input => {
        expect(getInputPattern(MemoryDrillMode.IPV4_BINARY_TO_DECIMAL, input)).toBe(false);
      });
    });
  });
  ...
  describe('Mode Display Names', () => {
    ...
    test('should return correct display names for all modes', () => {
      ...
      Object.entries(expectedNames).forEach(([mode, expectedName]) => {
        expect(getModeDisplayName(mode)).toBe(expectedName);
      });
    });
  });
  ...
  describe('Input Placeholders', () => {
    ...
    test('should return correct placeholders for all modes', () => {
      ...
      Object.entries(expectedPlaceholders).forEach(([mode, expectedPlaceholder]) => {
        expect(getInputPlaceholder(mode)).toBe(expectedPlaceholder);
      });
    });
  });
  ...
  describe('Prefix Selection Logic', () => {
    test('should handle prefix selection updates', () => {
      ...
      localStorageMock.setItem('memory-drill-selected-prefixes', JSON.stringify(newPrefixes));
